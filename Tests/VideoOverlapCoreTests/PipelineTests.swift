...
@Suite("RTMPose decoding")
struct RTMPoseDecodingTests {

    /// SimCC decoding is pure arithmetic and easy to get subtly wrong in a way
    /// that produces plausible, incorrect keypoints — a failure no confidence
    /// number would reveal. Pinned here without needing the model.
    @Test("Argmax over both axes recovers a planted peak")
    func decodesPlantedPeak() {
        let k = 3, xBins = 384, yBins = 512
        var x = [Float](repeating: 0, count: k * xBins)
        var y = [Float](repeating: 0, count: k * yBins)
        // Keypoint 1 peaks at x bin 100, y bin 200 → 50, 100 after the split.
        x[1 * xBins + 100] = 9
        y[1 * yBins + 200] = 7
        let (points, scores) = RTMPoseDecoding.decodeSimCC(simccX: x, simccY: y, keypointCount: k)

        #expect(points.count == k)
        #expect(abs(points[1].x - 50) < 1e-6)
        #expect(abs(points[1].y - 100) < 1e-6)
        // rtmlib scores a keypoint by the *lower* of the two axis peaks.
        #expect(abs(scores[1] - 7) < 1e-6)
        // Flat channels have no positive peak and must score zero rather than
        // landing a joint at the top-left corner.
        #expect(scores[0] == 0)
    }

    @Test("Box transform pads and matches the model aspect without cropping")
    func boxTransform() {
        // A tall box, as a climber usually is.
        let t = RTMPoseDecoding.boxTransform(for: CGRect(x: 100, y: 100, width: 40, height: 200))
        let aspect = Double(RTMPoseDecoding.inputWidth) / Double(RTMPoseDecoding.inputHeight)
        #expect(abs(Double(t.scale.width) / Double(t.scale.height) - aspect) < 1e-9)
        #expect(t.centre == CGPoint(x: 120, y: 200))
        // The long side is padded, never cut — cropping would lose a reaching
        // hand, which is the joint that matters most.
        #expect(Double(t.scale.height) >= 200 * 1.25 - 1e-9)
    }

    @Test("A centred keypoint maps back to the box centre")
    func roundTripsThroughSourcePixels() {
        let box = CGRect(x: 100, y: 50, width: 80, height: 300)
        let t = RTMPoseDecoding.boxTransform(for: box)
        let centreOfInput = CGPoint(
            x: Double(RTMPoseDecoding.inputWidth) / 2,
            y: Double(RTMPoseDecoding.inputHeight) / 2
        )
        let mapped = RTMPoseDecoding.toSourcePixels(centreOfInput, transform: t)
        #expect(abs(mapped.x - box.midX) < 1e-6)
        #expect(abs(mapped.y - box.midY) < 1e-6)
    }

    /// The y flip happens in exactly one place. Getting it wrong inverts every
    /// skeleton while leaving aggregate statistics looking reasonable.
    @Test("Joints land in Vision's bottom-left normalized space")
    func normalizesLikeVision() {
        var points = [CGPoint](repeating: .zero, count: 17)
        var scores = [Double](repeating: 1, count: 17)
        // Left shoulder near the top of a 1000px-tall frame.
        points[5] = CGPoint(x: 250, y: 100)
        // Left hip near the bottom.
        points[11] = CGPoint(x: 250, y: 900)
        scores[0] = 0  // nose not found

        let joints = RTMPoseDecoding.joints(
            from: points, scores: scores, imageSize: CGSize(width: 500, height: 1000)
        )
        let shoulder = joints[.leftShoulder]
        let hip = joints[.leftHip]
        #expect(shoulder != nil && hip != nil)
        #expect(abs((shoulder?.point.x ?? 0) - 0.5) < 1e-9)
        // Top of the frame in pixels is *high* y in Vision's space.
        #expect(abs((shoulder?.point.y ?? 0) - 0.9) < 1e-9)
        #expect(abs((hip?.point.y ?? 0) - 0.1) < 1e-9)
        #expect(joints[.nose] == nil, "a zero-score keypoint must not be emitted")
        // neck and root are derived, since COCO has neither.
        #expect(joints[.neck] != nil)
        #expect(joints[.root] != nil)
    }
}
